...
<div class="mb-body">
  ...
  <!-- ITEM COORDINATES -->
  <div class="mb-box">
    <div class="mb-head"><i class="fa fa-list"></i> <?php _e('Messages in thread', 'instant_messenger') . ' #' . im_arr($thread, 'i_thread_id'); ?></div>

    <div class="mb-inside">
      <div class="mb-info-box">
        <div class="mb-line">
          <div class="mb-col-6"><?php _e('Thread Title', 'instant_messenger'); ?>:</div>
          <div class="mb-col-18"><strong><?php echo im_arr($thread, 's_title'); ?></strong></div>
        </div>

        <div class="mb-line">
          <div class="mb-col-6"><?php _e('Questioner (asking question)', 'instant_messenger'); ?>:</div>
          <div class="mb-col-18"><strong><?php echo im_arr($thread, 's_from_user_name'); ?></strong> (<?php echo im_arr($thread, 's_from_user_email'); ?>)</div>
        </div>

        <div class="mb-line">
          <div class="mb-col-6"><?php _e('Respondent (seller)', 'instant_messenger'); ?>:</div>
          <div class="mb-col-18"><strong><?php echo im_arr($thread, 's_to_user_name'); ?></strong> (<?php echo im_arr($thread, 's_to_user_email'); ?>)</div>
        </div>

        <div class="mb-line">
          <div class="mb-col-6"><?php _e('Listing', 'instant_messenger'); ?>:</div>
          <div class="mb-col-18"><strong><?php
            if(isset($item['pk_i_id'])) {
              echo $item['s_title'];
            } else if((int)im_arr($thread, 'fk_i_item_id', 0) > 0) {
              echo __('Listing removed', 'instant_messenger') . ' (#' . (int)$thread['fk_i_item_id'] . ')';
            } else {
              echo __('Direct message', 'instant_messenger');
            }
          ?></strong></div>
        </div>
      </div>
        ...
        <?php foreach($messages as $m) { ?>
          <div class="mb-table-row <?php echo ($m['i_type'] == 0 ? 'im-from' : 'im-to'); ?>">
            ...
            <div class="mb-col-4 mb-align-left mb-att">
              <?php if($m['s_file'] <> '') { ?>
                <a href="<?php echo im_attachment_url($thread['i_thread_id'], $m['s_file']); ?>" target="_blank"><?php echo $m['s_file']; ?></a>
              <?php } ?>
            </div>
