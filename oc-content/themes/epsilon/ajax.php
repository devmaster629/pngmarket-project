      ...
      if($lookupPart != '') {
        ...
        if(is_array($listingCandidates)) {
          foreach($listingCandidates as $item) {
            ...
            // Every typed fragment must occur somewhere in the title.
            $allPartsFound = true;
            foreach($termParts as $part) {
              $position = function_exists('mb_strpos') ? mb_strpos($normalizedTitle, $part, 0, 'UTF-8') : strpos($normalizedTitle, $part);
              if($position === false) {
                $allPartsFound = false;
                break;
              }
